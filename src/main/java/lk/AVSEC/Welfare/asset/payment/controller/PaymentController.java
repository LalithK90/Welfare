package lk.AVSEC.Welfare.asset.payment.controller;
...
import lk.AVSEC.Welfare.asset.payment.service.PaymentService;
...
public class PaymentController {
    ...
    public PaymentController(PaymentService paymentService, lk.AVSEC.Welfare.asset.PurchaseOrder.service.PurchaseOrderService purchaseOrderService,
    ...
    public String getAllPurchaseOrderToPay(Model model) {
        ...
        List<lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder> purchaseOrdersDB =
                purchaseOrderService.findByPurchaseOrderStatus(lk.AVSEC.Welfare.asset.PurchaseOrder.entity.Enum.PurchaseOrderStatus.NOT_PROCEED);
        if ( !purchaseOrdersDB.isEmpty() ) {
            //need to pay po
            List<lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder> purchaseOrders = new ArrayList<>();

            for ( lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder purchaseOrder : purchaseOrdersDB ) {
    ...
    public String addPaymentAmount(@PathVariable( "id" ) Integer id, Model model) {
        ...
        lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder purchaseOrderNeedToPay = purchaseOrderService.findById(id);

        //1. still not processed po 2. partially paid po
        List<lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder> purchaseOrdersDB =
                purchaseOrderService.findByPurchaseOrderStatusAndSupplier(lk.AVSEC.Welfare.asset.PurchaseOrder.entity.Enum.PurchaseOrderStatus.NOT_PROCEED,
                                                                          purchaseOrderNeedToPay.getSupplier());
        List<lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder> purchaseOrderNotPaid = new ArrayList<>();
    ...
    public String savePayment(@Valid @ModelAttribute Payment payment, BindingResult bindingResult) {
        ...
        lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder purchaseOrder = paymentDB.getPurchaseOrder();
        ...
        if ( !payments.isEmpty() ) {
            ...
            if ( paidAmount.equals(purchaseOrderPrice) ) {
                ...
                lk.AVSEC.Welfare.asset.PurchaseOrder.entity.PurchaseOrder completedPurchaseOrder =purchaseOrderService.findById(purchaseOrder.getId());
                completedPurchaseOrder.setPurchaseOrderStatus(lk.AVSEC.Welfare.asset.PurchaseOrder.entity.Enum.PurchaseOrderStatus.COMPLETED);
